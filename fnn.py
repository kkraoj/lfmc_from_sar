...
def build_data(df, ignore_multi_spec = False, delta_days = delta_days, \
    ...
    dont_norm = df['percent']
    df = (df - df.mean())/(df.std())    #normalize 
    df['percent'] = dont_norm
    train, test = train_test_split(df, train_size = 0.6, test_size = 0.4)
    train_y= train['percent']
    train_x= train.drop(['percent'],axis = 'columns')
    test_y= test['percent']
    test_x= test.drop(['percent'],axis = 'columns')
...
def build_model(n_features):
	# create model
    reg = regularizers.l2(2)
    model = Sequential()
    model.add(Dense(150, input_dim = n_features,kernel_initializer='normal', activation='relu',kernel_regularizer=reg))
...
train_x, train_y, test_x, test_y, n_features =\
 build_data(df, ignore_multi_spec = True, delta_days = delta_days)
...
rmse_diff = infer_importance(model, train_x, train_y, test_x, test_y, retrain = False)
...
def plot_pred_actual(test_y, pred_y, R2):
    ...
    plot = ax.scatter(x,y, c=groups, s=8, edgecolor='', cmap = "Set1")
    axis_lim = [-10,310]
    ...
    ax.set_ylabel('Predicted fuel moisture')
    ax.set_xlabel('Actual fuel moisture')
    ...
    fig.colorbar(plot,ax=ax,cax=cax, ticks = np.linspace(1.4,8.6,9))
...
def plot_importance(rmse_diff):
    ...
    ax.set_xlabel('RMSE Shift (% fuel moisture)')
    green = '#1b9e77'
    brown = '#d95f02'
    blue = '#7570b3'
...
def append_color_importance(Df):
    ...
    topo=['slope', 'elevation', 'canopy_height','latitude', 
          'longitude','silt', 'sand', 'clay', 'incidence_angle']
    Df['color']=None
