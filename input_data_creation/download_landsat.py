...
def getArgs():
   ...
   parser.add_argument('-cloudfree',
                       '--cloudfree',
                       help = '''download cloud free pixels only. default = False''',
                       default = False)  
...
#    select(tuple(bands))## for sentinel-2
#   
#   ###mask clouds if requested
#   if args.cloudfree=='True':
#      print('cloudfree requested')
#      col = ee.ImageCollection(product).\
#          filterDate(start_date, end_date).\
#          map(cloudMask).select(tuple(bands))
...
#        for (start, end) in split_years(args.start, args.end):
