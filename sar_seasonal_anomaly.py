...
#    if len(obs_sub.loc[obs_sub.index<='2017-01-01',:])<10  or site not in meas.site.values:
#        continue
#    elif site in meas.site.values:
#        meas_sub  = meas.loc[meas.site==site,:]
#        if len(meas_sub.loc[meas_sub.index>='2017-01-01',:])<10 or len(meas_sub.fuel.unique())>1:
#            continue
...
##    obs_sub[var].plot(ax = ax, marker = 'o', ms = 3, ls='none', c= 'y',mfc = "none",mew = 0.5,  mec = "y", label = 'loess')
...
##    meas_sub.percent.plot(ax = ax2, marker = 'o', ms = 3, ls='-', mfc = "none",mew = 0.5,  c = "maroon",mec = "maroon", label = 'fm')
##
###    ax2.plot(x2_,y2_, color = c2, lw = 1)
##    corr = pd.Series(y_,x_).corr(pd.Series(y2_,x2_))
##    ax.annotate('corr = %0.2f'%corr, xy=(1, 0.84),ha = "right", xycoords='axes fraction', color = 'y')  
##    
##    ax3 = ax.twinx()
##    
...
#fig, ax = plt.subplots(figsize= (4,1.5))
#mean.plot(ax = ax, label = 'mean', color = 'k', lw = 1, ls = '--')
#absolute.plot(ax = ax, label = 'raw', color = 'k')
#ax.fill_between(mean.index, mean.values, absolute.values, where=absolute.values <= mean.values,\
#                color = 'darkslateblue', label = '-anomaly')
#ax.fill_between(mean.index, mean.values, absolute.values, where=absolute.values> mean.values,\
#                color = 'olivedrab', label = '+anomaly')
#ax.set_ylabel('FM (%)')
